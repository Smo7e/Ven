class OneWayHyperboloid extends Figure {
    constructor(a = 2, b = 3, c = 4, count = 20, color = "lightgreen", x = 0, y = 0, z = 0) {
        const points = [];
        const edges = [];
        const polygons = [];
        ...
        for (let i = 0; i < points.length; i++) {
            ...
            if (i + count < points.length && (i < count * count - count || i >= count * count)) {
                edges.push(new Edge(i, count + i));
            }
        }
        for (let i = 0; i < count; i++) {
            edges.push(new Edge(count * count - (i + 1) * count, count * count - 1 - count * i));
            edges.push(
                new Edge(points.length - 1 - count * i, count * count + count * count - count * (i + 1))
            );
        }
